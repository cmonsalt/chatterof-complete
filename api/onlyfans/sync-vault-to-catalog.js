...
export default async function handler(req, res) {
  ...
  try {
    ...
    // 1. Obtener todos los medias del vault
    let allMedias = [];
    let offset = 0;
    const limit = 50;
    let hasMore = true;
    ...
    // 2. Sincronizar metadata al catalog (sin descargar archivos)
    let syncedCount = 0;
    let skippedCount = 0;
    let errorCount = 0;
    ...
    for (const media of allMedias) {
      try {
        // Validar que el media esté listo
        ...
        const catalogData = {
          ...
          media_url: mediaUrl, // URL temporal (se refrescará cuando se necesite)
